---
title: 'Genomic Repeat inference from Depth (GRiD): A Pipeline for Haplotype-Resolved KIV-2 VNTR Copy Number Estimation in the LPA Gene'
tags:
  - Python
...
Lipoprotein(a) [Lp(a)] is a causal, independent, and highly heritable ($\text{h}^2 >70\%$) cardiovascular disease (CVD) risk factor that is elevated in an estimated 1.5 billion people globally [@tsimikas2017; @kronenberg2022; @kamstrup2021lipoprotein]. Elevated Lp(a) is a genetically established risk factor for coronary artery disease, myocardial infarction, aortic valve stenosis, and stroke [@clarke2009; @kamstrup2010; @nordestgaard2010; @guddeti2020lipoprotein; @arsenault2014lipoprotein; @marcovina1998lipoprotein]. Plasma Lp(a) concentrations are predominantly determined by copy number variation (CNV) in the Kringle IV type-2 (KIV-2) variable number tandem repeat (VNTR) within the *LPA* gene [@utermann1987]. The KIV-2 repeat unit spans approximately 5.5 kb and individuals carry 1–40 copies per haplotype, making the total VNTR region one of the largest and most structurally complex in the human genome and rendering KIV-2 CNV the single most important genetic determinant of Lp(a) levels [@mukamel2021; @kronenberg2022; @kamstrup2021lipoprotein]. Critically, KIV-2 copy number is inversely correlated with Lp(a) concentration: individuals with fewer copies tend to have higher Lp(a) and elevated CVD risk [@utermann1987; @clarke2009; @kamstrup2010]. Accurately measuring KIV-2 CNV from whole-genome sequencing (WGS) is central to building accurate cross-population Lp(a) polygenic risk scores (PRS) and to enabling the causal inference studies needed to characterize Lp(a)'s phenotypic effects and inform therapeutic strategies across diverse populations [@mukamel2021; @tsimikas2017].

**GRiD** (**G**enomic **R**epeat **i**nference from **D**epth) is an open-source Python pipeline that addresses this measurement gap. GRiD estimates diploid and haplotype-resolved VNTR copy number from short-read WGS data by combining read-depth normalization, read depth profile-matched nearest-neighbor estimation, and identity-by-descent (IBD)-based haplotype inference [@hujoel2026]. While GRiD was developed and validated for the *LPA* KIV-2 locus its core read-depth normalization and neighbor-based copy number framework is locus-agnostic and applicable to any large VNTR. The pipeline supports multi-threading and is driven by a single YAML configuration file, making it accessible to researchers without extensive bioinformatics expertise.

# Statement of Need

Accurate haplotype-resolved KIV-2 copy number estimation is a prerequisite for understanding the full genetic architecture of Lp(a) variation. Although several tools have recently addressed parts of this problem, none provides the complete workflow (standard short-read WGS to haplotype-resolved copy number estimates) in an open-source, cohort-scalable, and well-documented form. KILDA [@molitor2025] estimates diploid KIV-2 copy number directly from FASTQ files using an alignment-free k-mer approach, but does not resolve haplotype-specific copy numbers. The Illumina DRAGEN LPA Caller offers KIV-2 estimation on the proprietary DRAGEN platform but is not open-source and requires specific hardware. Kivvi [@pacbio2026kivvi] enables KIV-2 copy number estimation from PacBio HiFi long-read data, but long-read WGS remains unavailable for the large population cohorts where most epidemiological Lp(a) research is currently conducted. vntr-calling-nf [@vntrcallingnf] provides a Nextflow-based signature-sequence pipeline for VNTR variant calling but targets coding variant detection rather than copy number quantification, and has been primarily validated on whole-exome sequencing data. Researchers studying Lp(a) at cohort scale with short-read WGS have therefore continued to rely on convoluted, opaque, piecemeal, ad hoc scripts or unpublished internal pipelines, creating barriers to reproducibility and cross-cohort comparability. GRiD addresses this gap by providing a single, installable, documented tool that encapsulates the entire estimation workflow in a reproducible, multi-threaded, and configurable pipeline.

The *LPA* KIV-2 locus presents biological and computational characteristics that place it outside the scope of generic repeat genotyping tools. Its 5.6 kb repeat unit is orders of magnitude larger than the short tandem repeats (STRs, 1–6 bp motifs) targeted by most existing software. The VNTR region itself can span over 200 kb in individuals with high copy number, making read-pair spanning approaches infeasible with standard 150 bp paired-end reads. Critically, obtaining haplotype-resolved rather than diploid copy number estimates requires a phasing strategy that does not depend on long reads or prior genotyped variants, a requirement uniquely addressed by the IBD-based algorithm [@hujoel2026] implemented in GRiD. Beyond *LPA*, GRiD's read-depth normalization and nearest-neighbor framework are locus-agnostic and applicable to any large VNTR; @mukamel2021 identified 734 protein-coding VNTRs with elevated IBD rates across the human genome, all of which represent potential targets.
...
For the *LPA* KIV-2 locus specifically, KILDA [@molitor2025] provides an alignment-free diploid copy number estimate from short-read FASTQ files using a k-mer counting strategy calibrated against long-read assemblies. While KILDA demonstrates strong concordance with assembly-based diploid estimates, it does not address haplotype-level phasing. The foundational algorithmic framework integrated by GRiD was established by @mukamel2021, who demonstrated that population-normalized read depth is the most tractable approach for copy number estimation from short-read cohort data, and by @hujoel2026, who developed an IBD-based iterative algorithm for decomposing diploid copy number into haplotype-specific contributions without requiring long reads or pre-existing phased variant data. GRiD integrates and operationalizes both methodological contributions into a single accessible software package, read depth profile-aware nearest-neighbor normalization, a full configuration system, and multi-cohort scalability.
...
2. **Read counting**: Counts properly paired reads in the KIV-2 VNTR region using pysam [@pysam], filtering by mapping quality and SAM flag.
...
GRiD has attracted national and international interest from research groups across multiple conferences, with inquiries from groups seeking to apply KIV-2 copy number estimation to their own short-read WGS cohorts. It is designed for accessibility, with comprehensive documentation of configuration, outputs, and algorithmic details to lower the barrier to entry for epidemiologists and clinicians without extensive bioinformatics training.
