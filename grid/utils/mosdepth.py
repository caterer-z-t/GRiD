...
def compute_mosdepth(
   ...
   # Check mosdepth availability
    ...
    with progress_bar(console, total=len(files), description="Running mosdepth") as (progress, task):
...
def build_mosdepth_command(
    ...
    """
    cmd = ["mosdepth", "-n", "--by", str(by), "-f", str(ref_fasta), str(output_prefix), str(cram_path), "-t", str(threads)]
...
def wait_for_mosdepth_output(
    ...
    for attempt in range(max_attempts):
        ...
        log(console, f"Waiting for mosdepth output ({attempt + 1}/{max_attempts})...", style="warning")
...
def remove_intermediate_files(work_dir: Path, console=None, include_region_bed_gz: bool = False) -> None:
    """
