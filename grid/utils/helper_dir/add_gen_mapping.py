...
def main(args):
    ...
    genetic_map = pd.read_csv(args.genetic_map, sep="\s+", compression="gzip", comment="#")

    # Read MAP file
    map_file = pd.read_csv(args.map, sep="\t", names=["chr", "snp", "cm", "bp"])

    # Interpolate genetic positions for MAP file variants
    map_file["cm"] = np.interp(
        map_file["bp"], genetic_map["position"], genetic_map["Genetic_Map(cM)"]
    )

    # Save updated MAP file
    output_file = f"{args.out}.map"
    map_file.to_csv(output_file, sep="\t", header=False, index=False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Add genetic distances to PLINK MAP file using Eagle genetic map"
    )
