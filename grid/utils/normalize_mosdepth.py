...
def normalize_mosdepth(config, console):
    ...
    try:
        ...
        output_file_prefix = config.get('mosdepth', {}).get('normalize', {}).get('output_file_prefix', None)
    ...
    valid_regions = {
        region
        for region, mean_d in region_pop_means.items()
        if min_depth <= mean_d <= max_depth
    }
    ...
    regions_to_extract = {}
    with progress_bar(console, total=len(individuals), description="Extracting per-sample regions...") as (progress, task):

        with ThreadPoolExecutor(max_workers=max(1, threads)) as executor:
            futures_to_file = {
                executor.submit(process_func, ind_id): ind_id
    ...
    log(console, f"Mosdepth normalization complete. Results written to {output_path}", style="success")
...
def normalize_matrix(mat):
    ...
    mu_pos   = col_means > 0
    sqrt_mu  = np.where(mu_pos, np.sqrt(col_means, where=mu_pos,
                                         out=np.full_like(col_means, np.nan)), np.nan)
    ...
    variance_ratios = {
        ...
        if not np.isnan(var_ratio[i])
    }
 
    return mat, variance_ratios, col_means, col_vars  # expose means/vars for header

def select_high_variance_regions(variance_ratios: dict[int, float], top_frac: float = 0.9) -> list[int]:
    """
...
def write_normalized_output(
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        sel_ratios = np.where(
            sel_means > 0,
            ratio_mult * sel_vars / sel_means,
            np.nan
        )
 
    with gzip.open(output_file, "wt") as out:
        means_str = "\t".join(
            "NA" if np.isnan(v) else f"{v:.3f}" for v in sel_means
        )
        out.write(f"{N}\t{Rwant}\t{means_str}\n")
 
        ratios_str = "\t".join(
            "NA" if np.isnan(v) else f"{v:.3f}" for v in sel_ratios
        )
        ...
        for i, ind_id in enumerate(individuals_order):
            ...
            vals = [
                "NA" if np.isnan(mat[i, j]) else f"{mat[i, j]:.2f}"
...
def filter_empty_samples(regions_to_extract: dict[str, list[tuple[int, int, float]]], console=None):
    ...
    filtered = {
        sample: regions
        for sample, regions in regions_to_extract.items()
        if len(regions) > 0
    }
