...
def run_lpa_realignments(
    ...
    with Progress(
        ...
        task = progress.add_task(f"[cyan]Processing files...", total=len(cram_files))

        with ThreadPoolExecutor(max_workers=threads) as executor:
            # Submit all jobs
            future_to_cram = {
                executor.submit(process_func, cram_file): cram_file for cram_file in cram_files
            }
            ...
            for future in as_completed(future_to_cram):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        progress.console.print(f"[red]✗ {cram_file.name}: {error_msg}[/red]")

                except Exception as e:
                    failed += 1
                    failed_samples.append(cram_file.name)
                    progress.console.print(f"[red]✗ Failed to process {cram_file.name}: {e}[/red]")
