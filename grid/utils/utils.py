...
def has_index(file_path, file_type):
    ...
    allowed_file_types = {"CRAM": "crai", "BAM": "bai"}
    if file_type.upper() not in allowed_file_types.keys():
        return False

    if file_type.upper() == "CRAM":
        return os.path.exists(file_path + ".crai") or os.path.exists(
            file_path.replace(".cram", ".crai")
        )

    if file_type.upper() == "BAM":
        return os.path.exists(file_path + ".bai") or os.path.exists(
            file_path.replace(".bam", ".bai")
        )
...
def check_index(config, console=None):
    ...
    results = {"missing_file": [], "missing_index": [], "has_index": []}

    # ---- main ----
    samples = get_samples(samples_file)

    with progress_bar(console, total=len(samples), description="Checking indexes") as (
        progress,
        task,
    ):
        for sample in samples:
...
def create_index(config, console=None):
    ...
    with progress_bar(console, total=len(samples), description="Creating index") as (
        progress,
        task,
    ):
        for sample in samples:
    ...
    if config["index"].get("output_file_prefix") and (
        results["missing_file"] or results["missing_index"]
    ):
...
def create_region_string(chrom: str, start: int, end: int, console=None) -> str:
    ...
    if chrom and start is not None and end is not None:
        ...
        log(
            console,
            "You must provide either a full region or chromosome, start, and end separately.",
            style="danger",
        )
