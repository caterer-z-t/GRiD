...
# In[10]: Compute Diploid CN Command
# @cli.command()
...
#               help="Number of neighbors to use for normalization (default: 300)")
# @click.option("-c", "--chrom", default="6", help="Chromosome of the VNTR (default: 6)")
# @click.option("-s", "--start", default=160605062
#                 , type=int, help="Start position of the VNTR (default: 160605062)")
# @click.option("-e", "--end", default=160647661, type=int, help="End position of the VNTR (default: 160647661)")
# def compute_dipcn(read_counts_file, neighbors_file, output_dir, n_nbr, chrom, start, end):
#     """Compute diploid copy numbers for LPA KIV-2 repeats using neighbor-based normalization."""
#     from .utils.compute_dipcn import compute_dipcn_pipeline

#     print_banner()
#     try:
#         compute_dipcn_pipeline(
#             count_file=Path(read_counts_file).expanduser(),
#             neighbor_file=Path(neighbors_file).expanduser(),
#             output_prefix=str(Path(output_dir).expanduser() / f"hg38_{chrom}_{start}_{end}"),
#             n_neighbors=n_nbr,
#             console=console
#         )
#     except Exception as e:
#         console.print(f"[red]✗ Diploid CN computation failed: {str(e)}[/red]")
#         sys.exit(1)
...
def main():
    ...
    try:
        ...
        log(console, "\nPipeline interrupted by user", style="warning")
        sys.exit(130)
    except Exception as e:
