...
from grid.utils.hi_inference import (
    _read_dip_cn_file,
    _load_ibs_neighbors,
    _load_ibd_neighbors,
)
...
def test_read_dip_cn_file_basic(tmp_path):
    ...
    IDs, IRRs, IDtoInd = _read_dip_cn_file(str(f))
...
def test_read_neighbors_file_basic(tmp_path):
    ...
    f = _make_neighbors_file(
        tmp_path,
        [
            "1001\t1\t0\t1.5\t0.1\t1002\t2",  # sample 0 hap1 gets neighbor 2*1 + 2 - 1 = 3
        ],
    )
    result = _load_ibs_neighbors(f, IDtoInd, MAX_NBR=10)
    assert result[0] == [(3, 1.0)]  # (neighbor_hap_idx, weight)
...
def test_read_neighbors_file_respects_max_nbr(tmp_path):
    ...
    result = _load_ibs_neighbors(f, IDtoInd, MAX_NBR=1)
...
def test_read_neighbors_file_empty_after_header(tmp_path):
    ...
    f = tmp_path / "nbrs.txt"
    f.write_text("ID\thap\tnbr_ind\tcM_len\tcM_edge\tid_nbr\thap_nbr\n")
    result = _load_ibs_neighbors(str(f), IDtoInd, MAX_NBR=10)
    assert result == [[], []]
